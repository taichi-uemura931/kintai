...
<div class="attendance-list-container">
    ...
    <div class="month-switcher">
        <a href="{{ route('admin.staff.attendance', ['id' => $user->id, 'month' => \Carbon\Carbon::parse($yearMonth)->subMonth()->format('Y-m')]) }}">← 前月</a>
        <div class="calendar-wrapper">
            <span class="calendar-icon" onclick="document.getElementById('calendar-input').click()">📅</span>
            <span class="calendar-label">{{ \Carbon\Carbon::parse($yearMonth)->format('Y年m月') }}</span>
            <input type="text" id="calendar-input" readonly style="display: none;" onfocus="this.blur();" />
        </div>
        <a href="{{ route('admin.staff.attendance', ['id' => $user->id, 'month' => \Carbon\Carbon::parse($yearMonth)->addMonth()->format('Y-m')]) }}">翌月 →</a>
    </div>
    ...
    <form action="{{ route('admin.staff.attendance.csv', ['id' => $user->id, 'month' => $yearMonth]) }}" method="GET">
...
document.addEventListener('DOMContentLoaded', function () {
    flatpickr("#calendar-input", {
        ...
        onChange: function (selectedDates, dateStr) {
            window.location.href = "{{ route('admin.staff.attendance', ['id' => $user->id]) }}?month=" + dateStr.replace('/', '-');
        }
    });
});
</script>
